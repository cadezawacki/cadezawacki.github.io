// ppc-make-login.mjs — generates the two login ciphertexts embedded in ppc.html.
//
//   node tools/ppc-make-login.mjs
//
// Each ciphertext is the Firebase database URL encrypted with one login word.
// Wire format is byte-compatible with ppc.html's decryptText (same as txt.html):
...
//
// Paste the printed values into LOGIN_CTS in ppc.html. Re-run only if the
// database URL or a login word changes.
